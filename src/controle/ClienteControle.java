...
public class ClienteControle extends AbstractTableModel {
    ...
    public ClienteKvm getBean(int linha){
        //System.out.println("getbean");
        return (ClienteKvm) lista.get(linha);
    }
    @Override
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        ClienteKvm cliente = (ClienteKvm) lista.get(rowIndex);
        //System.out.println("getvalueat" + rowIndex + "e" + columnIndex);

        if (columnIndex == 0) {
            return cliente.getIdclienteKvm();
        }
        if (columnIndex == 1) {
            return cliente.getNomeKvm();
        }
        if (columnIndex == 2) {
            return cliente.getCpfKvm();
        }
        if (columnIndex == 3) {
            return cliente.getCidadeKvm();
        }
